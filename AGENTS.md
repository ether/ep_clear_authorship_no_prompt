# Agent Guide — ep_clear_authorship_no_prompt

Supresses the clear authorship prompt.
...
`ep_clear_authorship_no_prompt` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_clear_authorship_no_prompt`:
